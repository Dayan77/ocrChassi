    ...
    for img_path in image_paths:
        ...
        for detected_box in boxes:
            ...
            if best_match:
                ...
                h, w = char_img.shape
                # Pad to square to match InferenceView logic
                if h > w:
                    pad = (h - w) // 2
                ...
                # Scale image to model dimensions
                if char_img.shape[0] != img_h or char_img.shape[1] != img_w:
...
def train_easyocr_custom(model_data, progress_callback, log_callback):
    ...
    # ensure alphanumeric default characters are included
    default_chars = [str(i) for i in range(10)] + [chr(c) for c in range(ord('A'), ord('Z')+1)]
    if set(class_names) != set(default_chars):
        log_callback("Updating EasyOCR class list to include full alphanumeric set.")
        merged = sorted(set(class_names).union(default_chars))
        class_names = merged
    num_classes = len(class_names)
...
#                     return
#
#                 # Use safe model loader that handles class mismatch
...
#                     model, model_path, device, num_classes
#                 )
#                 if not success:
#                     self.log_area.append(f"Failed to load model: {message}")
#                     self.run_test_button.setEnabled(True)
#                     return
#
#                 if requires_retraining:
#                     self.log_area.append(f"Warning: {message}")
#
#                 model.eval()
#
#                 results = []
